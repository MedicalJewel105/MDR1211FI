...
int main()
{
	...
	int is_portF6TX = 0;
	while (1){
		if (MDR_TIMER1->STATUS & (1 << TIMER_STATUS_CNT_ARR_EVENT_Pos)){
			MDR_TIMER1->STATUS &= ~(1 << TIMER_STATUS_CNT_ARR_EVENT_Pos); // сброс флага совпадения
			
			is_portF6TX = !is_portF6TX;
			if (is_portF6TX){
				MDR_PORTF->RXTX |= (1 << 6);
			}
			else{
...
void timer_setup(void){
	// настройка пина А1 как выход таймера 1
	MDR_PORTA->ANALOG |= (1 << 1); // цифровой режим работы
	MDR_PORTA->OE |= (1 << 1); // включить выход
	MDR_PORTA->FUNC |= (2 << 2); // альт режим - TMR1_CH1
	MDR_PORTF->PWR |= (3 << 2); // питание максимально быстрый фронт
	...
	MDR_TIMER1->PSG = 0x00000007; //Предделитель частоты TIM_CLK
	MDR_TIMER1->ARR = 0xFFFFFFFF; //Основание счета (до 15)
